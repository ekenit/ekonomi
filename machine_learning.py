...
def extrapolate_future(df, number_of_months=12):
    ...
    future_flow = pd.DataFrame({'Month': future_months, 'Forecast': forecasts})
...
def linear_regression_forecast(df, number_of_months=12):
    ...
    future_months = pd.date_range(df['Month'].max(), periods=number_of_months + 1, freq='M')[1:]
